"""
Convert REPORT.md to a professionally formatted DOCX file.
...
def create_report():
    ...
    add_table(
        ...
        [
            ...
            ['Zou et al.', '2018', 'Applied Random Forest and neural networks for diabetes prediction; RF showed competitive accuracy.'],
            ['Kavakiotis et al.', '2017', 'Comprehensive review of ML and data mining in diabetes research; classification is the most prevalent technique.'],
            ['UCI ML Repository', '—', 'The Pima Indians Diabetes Dataset is a widely used benchmark for binary classification in medical ML.'],
        ]
    )
    add_para('Key takeaways:')
    add_bullet('**Random Forest** is among the most robust classifiers for small-to-medium-sized tabular healthcare datasets.')
    add_bullet('**Feature selection** (using clinically relevant features) improves both accuracy and interpretability.')
...
└─────────────────────────────────────────────┘"""
    ...
    add_para('Patient Data (Age, BP, Glucose, BMI)  →  [Disease Prediction System (Flask + ML)]  →  Risk Result (Yes/No, Probability %)')
    ...
    steps = [
        ('Step 1: Data Acquisition',
         'The Pima Indians Diabetes Dataset is downloaded programmatically from a public GitHub repository. '
         'The raw dataset contains 768 rows and 9 columns.'),
        ('Step 2: Feature Selection',
         'Only four clinically intuitive features are retained: Age, BloodPressure, Glucose, and BMI. '
         'The target variable is Outcome (0 = No Diabetes, 1 = Diabetes).'),
        ('Step 3: Data Cleaning',
         'Zero values in BloodPressure, Glucose, and BMI are biologically implausible and represent missing data. '
         'These are replaced with NaN and imputed using column-wise median imputation.'),
        ('Step 4: Feature Scaling',
         'StandardScaler from Scikit-learn is applied to normalize features to zero mean and unit variance.'),
        ('Step 5: Model Training',
         'Primary Model: Random Forest Classifier (n_estimators=100, max_depth=5). '
         'Baseline Model: Logistic Regression. '
         'Train-Test Split: 80% training, 20% testing (random_state=42).'),
        ('Step 6: Model Evaluation',
         'Models are evaluated using Accuracy Score, Precision, Recall, and F1-Score.'),
        ('Step 7: Model Serialization',
         'The trained Random Forest model and the fitted scaler are serialized to .pkl files using Joblib.'),
        ('Step 8: Deployment',
         'A Flask web server loads the serialized model and exposes a /predict REST endpoint. '
         'A modern web frontend sends data as JSON and displays results in real time.'),
    ]
    ...
    add_para(
        ...
        'Since these features cannot biologically be zero, this project replaces 0 with NaN and then '
        'imputes with the **median** of each column.'
    )
    add_code_block(
    ...
    add_para(
        '**Why median?** The median is robust to outliers and preserves the distribution shape '
        'better than the mean for skewed medical data.'
    )
    ...
    add_para(
        '**Algorithm Overview:** Random Forest is an ensemble learning method that constructs multiple '
        'decision trees during training and outputs the class that is the mode of the individual tree '
        'predictions. It reduces overfitting by averaging multiple trees, each trained on a random '
    ...
    add_para(
        ...
        'using the logistic (sigmoid) function. It serves as a simple baseline for comparison.'
    )
    ...
    add_heading('11.4 Classification Report', level=2)
    add_para(
        'The trained model produces a detailed classification report with Precision, Recall, and '
        'F1-Score for both classes (Diabetic and Non-Diabetic).'
    )
    add_code_block(
    ...
    add_table(
        ...
        [
            ['Data Pipeline', 'data/download_prep.py', 'Downloads Pima dataset, selects features, cleans missing values, saves CSV'],
            ['Model Training', 'model/train.py', 'Loads CSV, splits data, scales features, trains RF & LR, serializes models'],
            ['Web Server', 'app.py', 'Flask app with GET / (UI) and POST /predict (JSON prediction)'],
            ['Frontend', 'templates/index.html', 'Responsive UI with form inputs, AJAX call, animated result display'],
            ['Styling', 'static/style.css', 'Modern glassmorphism design with dark theme and animations'],
        ]
    )
    ...
    add_bullet('The **Random Forest Classifier** was selected as the production model due to its superior accuracy and ability to capture non-linear relationships.')
    add_bullet('The probability output (from predict_proba) adds nuance beyond binary classification.')
    ...
    advantages = [
        ...
        '**Real-Time Predictions:** Results are displayed instantly via AJAX (no page reloads).',
        '**Probabilistic Output:** Provides a confidence percentage, not just a yes/no label.',
        '**Modern UI:** Glassmorphism design with animations improves user experience.',
        '**Modular Architecture:** Data pipeline, model training, and deployment are independent modules.',
        '**Reproducible:** Fixed random seeds and version-pinned dependencies ensure reproducibility.',
    ]
    ...
    limitations = [
        '**Limited Features:** Uses only 4 features; adding more clinical variables could improve accuracy.',
        '**Dataset Size:** The Pima dataset has only 768 records, limiting generalizability.',
        '**Single Disease:** Currently supports only diabetes prediction.',
        '**No Authentication:** No login/security features — not suitable for production healthcare use.',
        '**Bias:** The Pima dataset was collected from a specific demographic (Pima Indian women).',
    ]
    ...
    future_items = [
        '**Multi-Disease Support:** Extend the system to predict heart disease, kidney disease, etc.',
        '**Additional Features:** Incorporate Insulin, HbA1c, family history for improved accuracy.',
        '**Deep Learning Models:** Experiment with neural networks for higher accuracy on larger datasets.',
        '**User Authentication:** Add login functionality for secure medical data handling.',
        '**Patient History Tracking:** Store prediction history for longitudinal monitoring.',
        '**Mobile Application:** Develop a companion mobile app for on-the-go screening.',
        '**Explainable AI (XAI):** Use SHAP or LIME for feature importance explanations.',
        '**Cloud Deployment:** Deploy on AWS/GCP/Azure for public accessibility.',
    ]
    ...
    add_para(
        'This project underscores how accessible and impactful ML-based healthcare tools can be, and '
        'highlights the importance of responsible deployment with proper disclaimers.'
    )
    ...
    print(f'\n📝 Remember to fill in:')
    print(f'   - Your Name, Roll Number, Branch on the cover page')
    print(f'   - Faculty/Guide name')
    print(f'   - Screenshots in Section 15')
